...
var handle = socket =>{
    ...
    socket.on('data',data=>{
        ...
        if(!isExist){ //不存在当前连接就保存
            ...
            logger.debug('websocket用在线设备id号名称数组：');
            logger.debug(JSON.stringify(onlineArr));
            logger.debug(JSON.stringify(onlineList));
                ...
                new Promise.all(getProjectInfo).then(result=>{
                    ...
                    result.map(v=>{
                        ...
                        if(online[v.project_id]){
                            ...
                            /*if(v.project_id == 24){
                                testIds.push(v);
                                logger.debug(JSON.stringify(testIds));
                                logger.debug(testIds.length);
                            }*/
        ...
        if(datastore.equipmentsMap[id]){
            ...
            datastore.equipmentsMap[id].heart_time = new Date().getTime(); // 设置数据收到的时间
            let nodeModel = {
                name: result.name,
...
setInterval(()=>{
    ...
    for(let e in datastore.equipmentsMap){
        ...
        if(time - v.heart_time > 60 * 1000 * 2) { //先从内存拿数据
            logger.info('删除缓存数据.................');
            logger.info(datastore.equipmentsMap[e]);
            delete datastore.equipmentsMap[e]; //删除缓存数据
            logger.info('删除后.................');
            logger.info(datastore.equipmentsMap[e]);
        }else {
            if(!v){
...
handle.sendGroup=(id,data)=>{
    return new Promise((resolve, reject) => {
        // logger.info('批量设置>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>' + id);
        // logger.info(new Date().toLocaleString() + ' : ' + count);
        if(clientList[id]){
            writeHEX(id, data, 1, function (result) { //写数据
                // callback&&callback(result);
                resolve(result);
...
function writeHEX(id,data,count,callback) {
    ...
    clientList[id].once('data',data=>{
        ...
        let isACKResponse=ParseData.isACKResponse(buf);
        let ackDataLength=ParseData.getACKLength(buf);
        logger.info('isACKResponse: %s, ackDataLength: %s', isACKResponse, ackDataLength);

        if(isACKResponse===1&&ackDataLength===52){ //云鹏，百得控制箱，用八个ID后面的那个字节表示心跳吧，0-心跳，1-ack
            clientResult[id].result = true; //收到回复
            clearTimeout(timeClear[id]);
    ...
    //两秒内收不到回复重试，一共重试5次
    timeClear[id] = setTimeout(function () {
        logger.info('clientResult[id]:');
        logger.info(clientResult[id]);
