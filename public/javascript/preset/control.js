...
require(['jquery', 'handlebars', 'dialog', 'flatpickr', 'dialogTemplate', 'io'], function ($, handlebars, dialog, flatpickr, dialogTemplate, io) {
    ...
    $('body').on('click', '.editBtn', function () {
        ...
        var $this = $(this),
            id = $this.data('id'),
            presetType = $this.data('type'),
            project_name = '修改预置 ' + $this.data('name'),
            relay = $(this).data('relay');
        var data = presetList[id];
        var $content = '' +
            //'<label class="radio-inline"><input type="radio" name="optionsRadios" id="optionWeek" value="0" checked>周模式</label>' +
            '<div class="form-group">' +
            // '<label for="name" class="col-sm-2">名称</label>' +
            '<div class="col-sm-8" style="margin: 0 0 10px 0; padding:0;">' +
            '<input type="text" value="' + data.name + '" class="form-control" id="presetName" placeholder="请输入名称">' +
            '</div></div><br>' +
            // '<div class="row"></div>' +
            '<div class="form-group">' +
            // '<label for="name" class="col-sm-2">详情</label>' +
            '<div class="col-sm-12" style="margin: 0 0 10px 0; padding:0;">' +
            '<textarea class="form-control" rows="3" id="presetDescribe"  placeholder="请输入详情">' + data.describe + '</textarea>' +
            '</div></div><p></p>' +
            // '<div class="row"></div>' +
            '<label class="radio-inline">' +
            '<input type="radio" name="optionsRadios" id="optionWeek" value="0" data-relay=' + relay + ' checked>周模式</label>' +
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i] && data[i].type === 'week') {
                        preset.week_conf = JSON.parse(preset.week_conf);
                        presetList[preset.id] = preset;
                    } else {
                        presetList[preset.id] = preset;
                    }
                }
